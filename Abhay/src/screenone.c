...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static HBITMAP hBitmap1 = NULL; // A1
	static HBITMAP hBitmap2 = NULL; // A1
	static HBITMAP hBitmap3 = NULL; // A1
	static HBITMAP hBitmap4 = NULL; // A1
	static HBITMAP hBitmap5 = NULL; // A1
	...
	case WM_CREATE:
		...
		hBitmap1 = LoadBitmap((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), MAKEINTRESOURCE(IDBITMAP1));
		hBitmap2 = LoadBitmap((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), MAKEINTRESOURCE(IDBITMAP2));
		hBitmap3 = LoadBitmap((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), MAKEINTRESOURCE(IDBITMAP3));
		hBitmap4 = LoadBitmap((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), MAKEINTRESOURCE(IDBITMAP4));
		hBitmap5 = LoadBitmap((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), MAKEINTRESOURCE(IDBITMAP5));

		if (hBitmap1 == NULL  || hBitmap2 == NULL || hBitmap3 == NULL || hBitmap4 == NULL || hBitmap5 == NULL)
		{
			MessageBox(hwnd, TEXT("Failed to load bitmap!"), TEXT("Error"), MB_ICONERROR);
		}
		//  PlaySound(TEXT("qwe.wav"), NULL, SND_FILENAME | SND_ASYNC);
		break;
	...
	case WM_PAINT:
		...
		if (rectangleDisplayed)
		{
			...
			if (textInRectDisplayed)
			{
				...
				HDC hdcMem1 = CreateCompatibleDC(hMemDC); // Using double buffer hMemDC
				HBITMAP hOldBitmap = (HBITMAP)SelectObject(hdcMem1, hBitmap1);
			...
			if (newRectangleDisplayed)
			{
				...
				if (textInRectDisplayed1)
				{
					...
					HDC hdcMem1 = CreateCompatibleDC(hMemDC); // Using double buffer hMemDC
					HBITMAP hOldBitmap = (HBITMAP)SelectObject(hdcMem1, hBitmap2);
					...
					// Draw the bitmap
					StretchBlt(hMemDC, destX, destY, destWidth, destHeight,
							   hdcMem1, 0, 0, bitmap2.bmWidth, bitmap2.bmHeight, SRCCOPY);
				...
				if (textInRectDisplayed2)
					{
						...
						HDC hdcMem1 = CreateCompatibleDC(hMemDC); // Using double buffer hMemDC
						HBITMAP hOldBitmap = (HBITMAP)SelectObject(hdcMem1, hBitmap3);
						...
						// Draw the bitmap
						StretchBlt(hMemDC, destX, destY, destWidth, destHeight,
						hdcMem1, 0, 0, bitmap3.bmWidth, bitmap3.bmHeight, SRCCOPY);
			...
			if (newRectangleDisplayed3)
			{
				...
				if (textInRectDisplayed4)
				{
					...
					HDC hdcMem1 = CreateCompatibleDC(hMemDC); // Using double buffer hMemDC
					HBITMAP hOldBitmap = (HBITMAP)SelectObject(hdcMem1, hBitmap5);
