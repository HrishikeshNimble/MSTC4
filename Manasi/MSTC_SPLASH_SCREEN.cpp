...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( SPLASH_SCREEN_RUN_STATUS == TRUE)  // if the current scene is splash screen, otherwise move to the next scene
    {
        if(SPLASH_SCREEN_BG_DRAW == 1)  // this is for checking that the logo's background should be drawn only once in paint 
        {
        ...
        {
            RandomPen(&HLinePen, &color, 8);  // Random pen for drawing
        }

        HOldPen = (HPEN)SelectObject(hdc, HLinePen);

        for(pointIndex = 0; pointIndex < SPLASH_SCREEN_POINTS_PLOTED; pointIndex++)  // Draws the circular bar around the logo image
        {
            if(pointIndex != 0)
            {
                MoveToEx(hdc, pVec_Splash_Screen_outer->list[pointIndex - 1]->xPosition + SPLASH_SCREEN_X_SHIFT_OUTER,
                          pVec_Splash_Screen_outer->list[pointIndex - 1]->yPosition + SPLASH_SCREEN_Y_SHIFT_OUTER, NULL);
                LineTo(hdc, pVec_Splash_Screen_outer->list[pointIndex]->xPosition + SPLASH_SCREEN_X_SHIFT_OUTER,
                       pVec_Splash_Screen_outer->list[pointIndex]->yPosition + SPLASH_SCREEN_Y_SHIFT_OUTER);
            }
        }

        if(SPLASH_SCREEN_POINTS_PLOTED == pVec_Splash_Screen_outer->count)  // This means the circle drawing has completed in the splash screen 
        {
			...
			HSplashScreenFont = CreateFont(40, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_TT_PRECIS, 
                                          CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, TEXT("Cambria"));
            HOldFont = (HFONT)SelectObject(hdc, HSplashScreenFont);

            // Displaying splash screen text
            TextOut(hdc, SPLASH_SCREEN_X_SHIFT_OUTER, SPLASH_SCREEN_Y_SHIFT_OUTER + SPLASH_SCREEN_CIRCLE_RADIUS - 480, 
                    "CoreCode Programming Academy LLP", lstrlen("CoreCode Programming Academy LLP"));
            TextOut(hdc, SPLASH_SCREEN_X_SHIFT_OUTER, SPLASH_SCREEN_Y_SHIFT_OUTER + SPLASH_SCREEN_CIRCLE_RADIUS + 30, 
                    "Group Decimals Presents : Reinventing The Wheels", lstrlen("Group Decimals Presents : Reinventing The Wheels"));

            // Set the splash screen completion flag if necessary
            // SPLASH_SCREEN_RUN_STATUS = FALSE;  // Uncomment this line if you want the screen to be empty immediately after splash screen completion
            // SPALSH_SCREEN_COMPLETED++;  // Increment completion counter if needed
        }
    }
    else 
    {
        SetTextAlign(hdc, TA_CENTER | TA_TOP); 
        SetBkMode(hdc, TRANSPARENT);
        SetTextColor(hdc, skyblue);  // Default color for other windows
