...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_SIZE:
        // Update the client rect when window is resized
    ...
    case WM_PAINT:
    {
        ...
        // Draw the white rectangle at the top (20% of window height)
        RECT topRect;
        topRect.left = 0;
        topRect.top = 0;
        topRect.right = rect.right;
        topRect.bottom = rect.bottom * 0.2; // 20% of window height

        HBRUSH hBrushWhite = CreateSolidBrush(RGB(255, 255, 255)); // White brush
        FillRect(hdcMem, &topRect, hBrushWhite);
        DeleteObject(hBrushWhite);

        // Create a bold font (scaled to window size)
        int fontSize = rect.bottom / 15; // Adjust divisor for desired size
        ...
        if (drawLines)
        {
            ...
            // Draw the code lines
            int codeFontSize = rect.bottom / 30; // Adjust for desired size
            ...
            // Calculate line spacing based on window height
            int lineSpacing = codeFontSize + 10;
            ...
            for (int i = 0; i <= currentCodeLine; i++)
            {
                ...
                codeRect.top = bottomY + 10 + (i * lineSpacing); // Dynamic spacing
                codeRect.right = centerX - 10;          // Stop before the vertical line
                codeRect.bottom = codeRect.top + codeFontSize;    // Height of the text
            ...
            for (int i = 0; i <= currentAssemblyLine; i++)
            {
                ...
                assemblyRect.top = bottomY + 10 + (i * lineSpacing);  // Dynamic spacing
                assemblyRect.right = rect.right - 10;        // 10px from the right edge
                assemblyRect.bottom = assemblyRect.top + codeFontSize; // Height of the text
