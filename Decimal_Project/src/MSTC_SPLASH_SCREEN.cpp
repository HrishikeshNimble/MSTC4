...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if( SPLASH_SCREEN_RUN_STATUS == TRUE)  //if the current scene is splash screen otherwise move to next scene
	{
				if(SPLASH_SCREEN_BG_DRAW == 1)  //this is for checking that the logo's background should be drawn only one in paint 
				{
				...
				for(pointIndex=0; pointIndex < SPLASH_SCREEN_POINTS_PLOTED ; pointIndex++)  //Draws the circular bar near aound logo image
				{
				...
				if( SPLASH_SCREEN_POINTS_PLOTED == pVec_Splash_Screen_outer->count ) //this means the circile drawing has completed in the splash screen 
				{
					//SetTextAlign(hdc, TA_LEFT | TA_TOP); 
					//SetBkMode(hdc,TRANSPARENT);
					//SetTextColor(hdc,skyblue);
					//TextOut(hdc, 0, 0, "Splash Screen Rendering has completed", lstrlen("Splash Screen Rendering has completed"));  
					
					//Above  is test message
					...
					TextOut(hdc, SPLASH_SCREEN_X_SHIFT_OUTER, SPLASH_SCREEN_Y_SHIFT_OUTER + SPLASH_SCREEN_CIRCLE_RADIUS + 30, "Hon. Linus Torvalds Group Presents : Music Encyclopedia", lstrlen("Hon. Linus Torvalds Group Presents : Music Encyclopedia"));

					//PostQuitMessage(0);
					//SPLASH_SCREEN_RUN_STATUS=FALSE;   //Uncomment this line if you want the screen to be empty immedtely after completion of splash screen circle
     ...
     case WM_TIMER:
		...
		if(SPALSH_SCREEN_COMPLETED == 1)  //to ensure that invalidate rect is called once after spalsh screen is done rendering
		{
